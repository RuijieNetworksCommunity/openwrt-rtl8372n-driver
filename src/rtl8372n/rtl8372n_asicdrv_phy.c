...
#define MMD_REG_SERDES_CONTROL   0x1Eu //30
#define MMD_REG_PHY_CONTROL      0x1fu
#define MMD_REG_AUTO_NEGOTIATION 0x07u

const rtk_uint32 REG_AN_RESET = 0x00u;
const rtk_uint32 REG_AN_ADV = 0x10u;
const rtk_uint32 REG_MGBASE_AN_CTRL = 0x20u;
const rtk_uint32 REG_GBCR = 0xA412u;
...
ret_t rtl8372n_phy_read(rtk_uint32 port_index, rtk_uint32 page, rtk_uint32 reg_addr, rtk_uint32 *pvalue)
{
    ...
    while (1) {
        // 检查忙状态位 (bit0)
        result = rtl8372n_getAsicRegBit(PHY_CTRL_REG, 0, &busy_flag);
        if(result != RT_ERR_OK) return result;

        // 检查操作状态位 (bit24)
        result = rtl8372n_getAsicRegBits(PHY_CTRL_REG, PHY_OP_STATUS_MASK, &op_status);
        if(result != RT_ERR_OK) return result;
...
ret_t rtl8372n_phy_readBits(rtk_uint32 port_index, rtk_uint32 page, rtk_uint32 reg_addr, rtk_uint32 bits, rtk_uint32 *pvalue)
{
    ...
    while (1) {
        ...
        // 检查是否超出32位范围
        if (++bitsShift == 32) return RT_ERR_INPUT; // RT_ERR_INPUT (无效位掩码)
    }

    // 特殊处理页面30 (双寄存器组合)
    if (page == MMD_REG_SERDES_CONTROL) {
        // 读取低位寄存器
        rtk_uint32 low_value;
        result = rtl8372n_phy_read(port_index, MMD_REG_SERDES_CONTROL, reg_addr, &low_value);
        if(result != RT_ERR_OK) return result;
        
        // 读取高位寄存器 (地址+1)
        rtk_uint32 high_value;
        result = rtl8372n_phy_read(port_index, MMD_REG_SERDES_CONTROL, reg_addr + 1, &high_value);
        if(result != RT_ERR_OK) return result;
...
ret_t rtl8372n_phy_writeBits(rtk_uint32 port_mask, rtk_uint32 page, rtk_uint32 reg_addr, rtk_uint32 bits, rtk_uint32 value)
{
    ...
    if (page == MMD_REG_SERDES_CONTROL) {
        ...
        for (int port_index = 0; port_index < 9; port_index++) {
            ...
            //读取当前32位值 (双寄存器组合)
            rtk_uint32 high_value;
            result = rtl8372n_phy_read(port_index, MMD_REG_SERDES_CONTROL, next_reg, &high_value);
            if(result != RT_ERR_OK) return result;
            ...
            // 写入高位寄存器
            result = rtl8372n_phy_write(1 << port_index, MMD_REG_SERDES_CONTROL, next_reg, new_high);
            if(result != RT_ERR_OK) return result;
            
            // 写入低位寄存器
            result = rtl8372n_phy_write(1 << port_index, MMD_REG_SERDES_CONTROL, reg_addr, new_low);
            if(result != RT_ERR_OK) return result;
        }
...
{
    ret_t result; 
    result = rtl8372n_phy_regbits_write(1 << port, MMD_REG_PHY_CONTROL, 0xA436, 0xFFFF, addr);
    if(result != RT_ERR_OK) return result;
    return rtl8372n_phy_regbits_write(1 << port, MMD_REG_PHY_CONTROL, 0xA438, 0xFF00, value);
}
...
rtk_uint32 uc1_sram_read_8b(rtk_uint32 port, rtk_uint32 addr)
{
    ...
    rtl8372n_phy_regbits_write((1 << port), MMD_REG_PHY_CONTROL, 0xA436, 0xFFFFLL, addr);
    rtl8372n_phy_regbits_read(port, MMD_REG_PHY_CONTROL, 0xA438, 0xFF00, &value);
    return value;
}
...
{
    ret_t result; 
    result = rtl8372n_phy_regbits_write(1 << port, MMD_REG_PHY_CONTROL, 0xB87C, 0xFFFF, addr);
    if(result != RT_ERR_OK) return result;
    return rtl8372n_phy_regbits_write(1 << port, MMD_REG_PHY_CONTROL, 0xB87E, 0xFF00, value);
}
...
rtk_uint32 uc2_sram_read_8b(rtk_uint32 port, rtk_uint32 addr)
{
    ...
    rtl8372n_phy_regbits_write((1 << port), MMD_REG_PHY_CONTROL, 0xB87C, 0xFFFFLL, addr);
    rtl8372n_phy_regbits_read(port, MMD_REG_PHY_CONTROL, 0xB87E, 0xFF00, &value);
    return value;
}
...
ret_t data_ram_write_8b(rtk_uint8 port, rtk_uint32 addr, rtk_uint32 value)
{
    ...
    rtl8372n_phy_regbits_write(1 << port, MMD_REG_PHY_CONTROL, 0xB88E, 0xFFFF, addr);
    // rtl8372n根据地址LSB确定写入位置
    if ( (addr & 1) != 0 )
        // 地址为奇数: 写入低8位
        return rtl8372n_phy_regbits_write(1 << port, MMD_REG_PHY_CONTROL, 0xB890, 0xFF, value);
    else
        // 地址为偶数: 写入高8位
        return rtl8372n_phy_regbits_write(1 << port, MMD_REG_PHY_CONTROL, 0xB890, 0xFF00, value);
}
...
ret_t rtl8372n_phy_common_c45_an_restart(rtk_uint32 port)
{
    ...
    result = rtl8372n_phy_read(port, MMD_REG_AUTO_NEGOTIATION, REG_AN_RESET, &reg_val);
    if (result != RT_ERR_OK) return result;

    if ( (reg_val & 0x1000) != 0 )
    {
        result = rtl8372n_phy_write(1LL << port, MMD_REG_AUTO_NEGOTIATION, REG_AN_RESET, reg_val | 0x200);
        if (result != RT_ERR_OK) return result;
    }
...
ret_t rtl8372n_phy_common_c45_autoNegoEnable_set(rtk_uint32 port, rtk_uint32 enable)
{
    ...
    result = rtl8372n_phy_write(1LL << port, MMD_REG_AUTO_NEGOTIATION, REG_AN_RESET, reg_val);
    return result;
}
...
ret_t rtl8372n_phy_autoNegoAbility_set(rtk_uint32 port, rtl8372n_autoNegoAbility_t *a2)
{
    ...
    result = rtl8372n_phy_read(port, MMD_REG_AUTO_NEGOTIATION, REG_AN_ADV, &reg_value);
    if (result != RT_ERR_OK) return result;

    reg_value = (((rtk_uint32)a2->pauseAbility << 9) & 0xC00) | // advertised asymmetric pause frames || advertised pause frames
                (((rtk_uint32)a2->speedDuplexAbility & 0xF) << 5) |   // 10/100|HALF/FULLL
                (reg_value & 0xFFFFF21Fu);
    result = rtl8372n_phy_write(port_mask, MMD_REG_AUTO_NEGOTIATION, REG_AN_ADV, reg_value); 
    if (result != RT_ERR_OK) return result;


    result = rtl8372n_phy_read(port, MMD_REG_AUTO_NEGOTIATION, REG_MGBASE_AN_CTRL, &reg_value);
    if (result != RT_ERR_OK) return result;

    reg_value = (((rtk_uint32)a2->speedDuplexAbility << 1) & 0x80) | //Advertise PHY as 2.5GBASE-T capable
                (reg_value & 0xFFFFEE7Fu);
    result = rtl8372n_phy_write(port_mask, MMD_REG_AUTO_NEGOTIATION, REG_MGBASE_AN_CTRL, reg_value);
    if (result != RT_ERR_OK) return result;


    result = rtl8372n_phy_read(port, MMD_REG_PHY_CONTROL, REG_GBCR, &reg_value);
    if (result != RT_ERR_OK) return result;

    reg_value = (((rtk_uint32)a2->speedDuplexAbility << 4) & 0x200) | //1000BASE-T full duplex capable
                (reg_value & 0xFFFFFDFFu);
    result = rtl8372n_phy_write(port_mask, MMD_REG_PHY_CONTROL, REG_GBCR, reg_value);
    if (result != RT_ERR_OK) return result;
